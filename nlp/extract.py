...
class Tender:
    def __init__(self, tender_id: str, organization: str, category: str, location: str, 
                 value: float, deadline: datetime, description: str, link: str, source: str = ""):
...
def process_tenders(raw_tenders: List[Dict]) -> List[Tender]:
    ...
    for tender in raw_tenders:
        # Handle both old format (with description) and new format (with title)
        description = tender.get("description", tender.get("title", ""))
        
        # Extract and normalize fields
        tender_id = extract_tender_id(tender.get("tender_id", "") + " " + description)
        organization = extract_organization(tender.get("organization", "") + " " + description)
        category = tender.get("category", "General")
        location = extract_location(tender.get("location", "") + " " + description)
